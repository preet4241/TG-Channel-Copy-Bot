...
async function loadSettings() { const result = await request('/api/settings'); if (!result.ok) return; ['complete','failed','flood'].forEach(key => { const input = document.getElementById(`setting-${key}`); if (input) input.checked = !!result.notification_settings[ key === 'complete' ? 'task_complete' : key === 'failed' ? 'task_failed' : 'flood_wait' ]; }); const auto = document.getElementById('setting-auto'); if (auto) auto.checked = !!result.auto_forward; const set = (id,value) => { const node = document.getElementById(id); if (node) node.textContent = value; }; const backup = result.persistence || {}; const last = backup.backup_last_upload_epoch ? new Date(backup.backup_last_upload_epoch * 1000).toLocaleString() : 'Never'; set('setting-max', result.max_task_messages); set('setting-storage', `${result.storage_limit_mb} MB`); set('setting-persistence', backup.backup_status || 'unknown'); set('setting-backup-interval', `${Math.round((backup.backup_interval_seconds || 300) / 60)} minutes`); set('setting-backup-last', last); set('setting-backup-status', backup.backup_status || 'unknown'); const error = document.getElementById('setting-backup-error'); if (error) { error.textContent = backup.backup_last_error ? `Last error: ${backup.backup_last_error}` : ''; error.classList.toggle('hidden', !backup.backup_last_error); } set('settings-source', appData.status.source || 'Not set'); set('settings-target', appData.status.target || 'Not set'); }
